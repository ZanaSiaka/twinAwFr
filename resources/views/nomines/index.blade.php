...
<div class="d-flex justify-content-end">
    ...
    <a href="{{ route('admin.nomine.create') }}" class="btn btn-success m-1">
...
<div class="">
    ...
    <div class="table-responsive">
            ...
            <tbody>
                @foreach($nomines as $nomine)
						<tr><td>{{ $nomine->id }}</td>
							...
							<td>{{ $nomine->user }}</td>
						<td>
                    <a href="{{ route('admin.nomine.show', ['id' => $nomine->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.nomine.edit', ['id' => $nomine->id]) }}" class="btn btn-success btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" data-id="{{ $nomine->id }}" class="btn btn-danger btn-sm deleteBtn">
        ...
        <div class="d-flex justify-content-center">
            {{ $nomines->links('pagination::bootstrap-5') }}
        </div>
    </div>
</div>
</div>
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/nomines/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/nomines/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
