        ...
        <table  id="Role" class="table">
            <thead>
                <tr>
                    ...
                    <th scope="col">Est visible</th>

						<th scope="col">Actions</th>
                </tr>
            </thead>
            <tbody>
                @foreach($chats as $nomine)
						<tr><td>{{ $nomine->id }}</td>
							<td>{{ $nomine->question }}</td>
                            <td>{{ $nomine->est_visible }}</td>
						<td>
                    <a href="{{ route('admin.chat.show', ['id' => $nomine->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.chat.edit', ['id' => $nomine->id]) }}" class="btn btn-success btn-sm">
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/chats/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/chats/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
