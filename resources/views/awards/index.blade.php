...
<div class="d-flex justify-content-end">
    ...
    <a href="{{ route('admin.award.create') }}" class="btn btn-success m-1">
                ...
                @foreach($awards as $award)
						<tr><td>{{ $award->id }}</td>
                            ...
                            <div class="form-group d-flex" id="preview_imageUrl" style="max-width: 100%;">
                                <img src="{{  Str::startsWith($award->imageUrl, 'http') ? $award->imageUrl : Storage::url($award->imageUrl) }}"
                                     alt="Prévisualisation de l'image"
                                     style="max-width: 100px; display: block;">
                            </div>
                        </td>
						<td>
                    <a href="{{ route('admin.award.show', ['id' => $award->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.award.edit', ['id' => $award->id]) }}" class="btn btn-success btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" data-id="{{ $award->id }}" class="btn btn-danger btn-sm deleteBtn">
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/awards/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/awards/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const tableHeaders = document.querySelectorAll('#Award th');
            ...
            tableHeaders.forEach(function(header, index) {
                ...
                const savedSelection = localStorage.getItem('selectedColumns#Award');
        ...
        function toggleColumn(columnIndex, show) {
            const dataTable = document.getElementById('Award');
        ...
        function saveSelection() {
            ...
            localStorage.setItem('selectedColumns#Award', JSON.stringify(selectedColumns));
        }
        ...
        function sortTable(columnIndex) {
            const table = document.getElementById('Award');
